...
function cofv = cov(A)
    % Replace with explicit calculation for older versions
    At = A(~isnan(A));
    S = std(At(:));
    M = mean(At(:));
    cofv = S / M;
...
function B1_plus_fields = initialiseFieldsMatrix(files, z_slices)
    fieldsCell = arrayfun(@(file) loadB1Plus(file), files, 'UniformOutput', false);
    B1_plus_fields = cat(4, fieldsCell{:});
...
function showField(fields, phases, z_index)
    ...
    imshow(total_field_abs, [], 'Colormap', hot);
end
